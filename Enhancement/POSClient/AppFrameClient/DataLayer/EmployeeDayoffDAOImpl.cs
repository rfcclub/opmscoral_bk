...
namespace AppFrame.DataLayer
{
    public class EmployeeDayoffDAOImpl : IEmployeeDayoffDAO
    {
        ...
        {
            return (EmployeeDayoff) HibernateTemplate.Get(typeof(EmployeeDayoff), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(EmployeeDayoff)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
