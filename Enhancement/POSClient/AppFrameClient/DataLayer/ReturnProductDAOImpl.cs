...
{
    public class ReturnProductDAOImpl : IReturnProductDAO
    {
        ...
        {
            return (ReturnProduct) HibernateTemplate.Get(typeof(ReturnProduct), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(ReturnProduct)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
