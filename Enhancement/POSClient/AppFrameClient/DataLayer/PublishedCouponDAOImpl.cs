...
{
    public class PublishedCouponDAOImpl : IPublishedCouponDAO
    {
        ...
        {
            return (PublishedCoupon) HibernateTemplate.Get(typeof(PublishedCoupon), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(PublishedCoupon)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
