...
{
    public class BlockInDetailDAOImpl : IBlockInDetailDAO
    {
        ...
        {
            return (BlockInDetail) HibernateTemplate.Get(typeof(BlockInDetail), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(BlockInDetail)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
