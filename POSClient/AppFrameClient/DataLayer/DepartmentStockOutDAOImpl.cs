...
{
    public class DepartmentStockOutDAOImpl : IDepartmentStockOutDAO
    {
        ...
        {
            return (DepartmentStockOut) HibernateTemplate.Get(typeof(DepartmentStockOut), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(DepartmentStockOut)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
