...
{
    public partial class CreateStockOutTemplateForm : BaseForm, IDepartmentStockInExtraView
    {
        ...
        public CreateStockOutTemplateForm()
        {
            InitializeComponent();
        }
...
//            }
        ...
        {
            IList<DepartmentStockInDetail> resultList = new List<DepartmentStockInDetail>();

            foreach (DepartmentStockInDetail detail in list)
            {
                DepartmentStockInDetail resDetail = GetInResultList(resultList, detail);
                if(resDetail == null)
                {
                ...
                {
                    resDetail.StockQuantity += detail.StockQuantity;
                    resDetail.Quantity += detail.Quantity;
                }
            }
            return resultList;
        }

        private DepartmentStockInDetail GetInResultList(IList<DepartmentStockInDetail> list, DepartmentStockInDetail detail)
        {
            foreach (DepartmentStockInDetail inDetail in list)
            {
                if(inDetail.Product.ProductMaster.ProductMasterId.Equals(
        ...
        private void WriteDepartmentList(Worksheet ws)
        {
           IList departmentList = new ArrayList();
            ...
            {
                for(int i = departmentList.Count;i<7;i++)
                {
            ...
            range.set_Value(Missing.Value,((Department)departmentList[0]).DepartmentId);
            range.AddComment(((Department)departmentList[0]).DepartmentName);

            // G
            range = ws.get_Range("G" + 1, "G" + 1);
            range.set_Value(Missing.Value, ((Department)departmentList[1]).DepartmentId);
            range.AddComment(((Department)departmentList[1]).DepartmentName);

            // H
            range = ws.get_Range("H" + 1, "H" + 1);
            range.set_Value(Missing.Value, ((Department)departmentList[2]).DepartmentId);
            range.AddComment(((Department)departmentList[2]).DepartmentName);
            
            // I
            range = ws.get_Range("I" + 1, "I" + 1);
            range.set_Value(Missing.Value, ((Department)departmentList[3]).DepartmentId);
            range.AddComment(((Department)departmentList[3]).DepartmentName);

            // J
            range = ws.get_Range("J" + 1, "J" + 1);
            range.set_Value(Missing.Value, ((Department)departmentList[4]).DepartmentId);
            range.AddComment(((Department)departmentList[4]).DepartmentName);

            // K
            range = ws.get_Range("K" + 1, "K" + 1);
            range.set_Value(Missing.Value, ((Department)departmentList[5]).DepartmentId);
            range.AddComment(((Department)departmentList[5]).DepartmentName);

            // L
            range = ws.get_Range("L" + 1, "L" + 1);
            range.set_Value(Missing.Value, ((Department)departmentList[6]).DepartmentId);
            range.AddComment(((Department)departmentList[6]).DepartmentName);
        }
