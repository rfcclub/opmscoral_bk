...
{
    public class CouponTypeDAOImpl : ICouponTypeDAO
    {
        ...
        {
            return (CouponType) HibernateTemplate.Get(typeof(CouponType), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(CouponType)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
