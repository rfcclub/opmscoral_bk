...
{
    public class DepartmentReturnCostDAOImpl : IDepartmentReturnCostDAO
    {
        ...
        {
            return (DepartmentReturnCost) HibernateTemplate.Get(typeof(DepartmentReturnCost), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(DepartmentReturnCost)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
