...
namespace AppFrame.DataLayer
{
    public class CouponDAOImpl : ICouponDAO
    {
        ...
        {
            return (Coupon) HibernateTemplate.Get(typeof(Coupon), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(Coupon)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
