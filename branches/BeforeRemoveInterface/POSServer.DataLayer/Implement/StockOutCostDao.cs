...
namespace POSServer.DataLayer.Implement
{
    public class StockOutCostDao : IStockOutCostDao
    {
		...
		public IList<TClass> FindAllSubProperty<TClass>(LinqCriteria<StockOutCost> criteria,Func<StockOutCost,TClass> subProp)
        {
            return (IList<TClass>)HibernateTemplate.Execute(
                                delegate(ISession session)
                                {
                                    ...
                                    QueryHandler<StockOutCost> handler = new QueryHandler<StockOutCost>(session);
                                    IList<StockOutCost> products = handler.GetList(criteria);
