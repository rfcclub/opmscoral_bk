    ...
    public class StockDefinitionStatusDao : IStockDefinitionStatusDao
    {
        ...
        public StockDefinitionStatus Add(StockDefinitionStatus data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Save("CoralPOS.Models.StockDefinitionStatus", data);
                        return data;
                    }
                );
        ...
        public int Update(StockDefinitionStatus data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Update("CoralPOS.Models.StockDefinitionStatus", data);
                        return 0;
                    }
                );
        ...
        public int Delete(StockDefinitionStatus data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Delete("CoralPOS.Models.StockDefinitionStatus", data);
                        return 0;
                    }
                );
		...
		public IList<TClass> FindAllSubProperty<TClass>(LinqCriteria<StockDefinitionStatus> criteria,Func<StockDefinitionStatus,TClass> subProp)
        {
            return (IList<TClass>)HibernateTemplate.Execute(
                                delegate(ISession session)
                                {
                                    ...
                                    QueryHandler<StockDefinitionStatus> handler = new QueryHandler<StockDefinitionStatus>(session);
                                    IList<StockDefinitionStatus> products = handler.GetList(criteria);
