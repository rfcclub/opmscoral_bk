    ...
    public class ReceiptOutDao : IReceiptOutDao
    {
        ...
        public ReceiptOut Add(ReceiptOut data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Save("CoralPOS.Models.ReceiptOut", data);
                        return data;
                    }
                );
        ...
        public int Update(ReceiptOut data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Update("CoralPOS.Models.ReceiptOut", data);
                        return 0;
                    }
                );
        ...
        public int Delete(ReceiptOut data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Delete("CoralPOS.Models.ReceiptOut", data);
                        return 0;
                    }
                );
		...
		public IList<TClass> FindAllSubProperty<TClass>(LinqCriteria<ReceiptOut> criteria,Func<ReceiptOut,TClass> subProp)
        {
            return (IList<TClass>)HibernateTemplate.Execute(
                                delegate(ISession session)
                                {
                                    ...
                                    QueryHandler<ReceiptOut> handler = new QueryHandler<ReceiptOut>(session);
                                    IList<ReceiptOut> products = handler.GetList(criteria);
