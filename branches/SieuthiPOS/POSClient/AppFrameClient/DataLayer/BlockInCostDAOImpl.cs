...
{
    public class BlockInCostDAOImpl : IBlockInCostDAO
    {
        ...
        {
            return (BlockInCost) HibernateTemplate.Get(typeof(BlockInCost), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(BlockInCost)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
