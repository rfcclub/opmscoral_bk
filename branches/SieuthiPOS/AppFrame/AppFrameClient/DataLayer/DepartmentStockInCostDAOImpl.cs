...
{
    public class DepartmentStockInCostDAOImpl : IDepartmentStockInCostDAO
    {
        ...
        {
            return (DepartmentStockInCost) HibernateTemplate.Get(typeof(DepartmentStockInCost), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(DepartmentStockInCost)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
