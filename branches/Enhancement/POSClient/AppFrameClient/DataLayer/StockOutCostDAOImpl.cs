...
{
    public class StockOutCostDAOImpl : IStockOutCostDAO
    {
        ...
        {
            return (StockOutCost) HibernateTemplate.Get(typeof(StockOutCost), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(StockOutCost)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
