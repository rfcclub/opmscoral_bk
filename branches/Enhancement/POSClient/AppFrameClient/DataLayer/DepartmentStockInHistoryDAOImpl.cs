...
{
    public class DepartmentStockInHistoryDAOImpl : IDepartmentStockInHistoryDAO
    {
        ...
        public DepartmentStockInHistory FindById(object id)
        {
            ...
            try
            {
                return (DepartmentStockInHistory)session.Get(typeof(DepartmentStockInHistory), id);
            }
            finally
            {
                session.Close();
            }
            //return (DepartmentStockInHistory) HibernateTemplate.Get(typeof(DepartmentStockInHistory), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(DepartmentStockInHistory)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
