...
namespace AppFrame.DataLayer
{
    public class CountryDAOImpl : ICountryDAO
    {
        ...
        {
            return (Country) HibernateTemplate.Get(typeof(Country), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(Country)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
