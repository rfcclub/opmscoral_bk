...
namespace AppFrame.DataLayer
{
    public class SupplierDAOImpl : ISupplierDAO
    {
        ...
        {
            return (Supplier) HibernateTemplate.Get(typeof(Supplier), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(Supplier)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
