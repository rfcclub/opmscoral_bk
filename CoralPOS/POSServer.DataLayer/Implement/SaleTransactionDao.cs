...
{
    public class SaleTransactionDaoImpl : ISaleTransactionDao
    {
        ...
        public IList<SaleTransaction> FindAll(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ...
                return hibernateCriteria.List<SaleTransaction>();
            }
            finally 
            {
                if (session != null)
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(SaleTransaction)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
