...
{
    public class ExProductColorDaoImpl : IExProductColorDao
    {
        ...
        public IList<ExProductColor> FindAll(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ...
                return hibernateCriteria.List<ExProductColor>();
            }
            finally 
            {
                if (session != null)
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(ExProductColor)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
