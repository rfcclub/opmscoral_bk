...
{
    public class MainftrDaoImpl : IMainftrDao
    {
        ...
        public IList<Mainftr> FindAll(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ...
                return hibernateCriteria.List<Mainftr>();
            }
            finally 
            {
                if (session != null)
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(Mainftr)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
