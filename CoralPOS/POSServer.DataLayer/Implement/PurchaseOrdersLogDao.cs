...
{
    public class PurchaseOrdersLogDaoImpl : IPurchaseOrdersLogDao
    {
        ...
        /// <returns></returns>
        public IList<PurchaseOrdersLog> FindAll(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ...
                return hibernateCriteria.List<PurchaseOrdersLog>();
            }
            finally 
            {
                if (session != null)
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(PurchaseOrdersLog)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
