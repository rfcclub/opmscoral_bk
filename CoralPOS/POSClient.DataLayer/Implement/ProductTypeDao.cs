...
namespace POSClient.DataLayer.Implement
{
    public class ProductTypeDao : IProductTypeDao
    {
		...
		public IList<TClass> FindAllSubProperty<TClass>(LinqCriteria<ProductType> criteria,Func<ProductType,TClass> subProp)
        {
            return (IList<TClass>)HibernateTemplate.Execute(
                                delegate(ISession session)
                                {
                                    ...
                                    QueryHandler<ProductType> handler = new QueryHandler<ProductType>(session);
                                    IList<ProductType> products = handler.GetList(criteria);
