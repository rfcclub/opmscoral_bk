...
namespace POSClient.DataLayer.Implement
{
    public class SaleTransactionDao : ISaleTransactionDao
    {
		...
		public IList<TClass> FindAllSubProperty<TClass>(LinqCriteria<SaleTransaction> criteria,Func<SaleTransaction,TClass> subProp)
        {
            return (IList<TClass>)HibernateTemplate.Execute(
                                delegate(ISession session)
                                {
                                    ...
                                    QueryHandler<SaleTransaction> handler = new QueryHandler<SaleTransaction>(session);
                                    IList<SaleTransaction> products = handler.GetList(criteria);
