...
namespace POSServer.BusinessLogic.Implement
{
	public class ProductTypeLogic : IProductTypeLogic
	{
		...
		public void Sync(IList<ProductType> productTypeList, IFlowSession flowSession)
		{
			IList<ProductType> allType =ProductTypeDao.FindAll(new LinqCriteria<ProductType>());
		    IList<ProductType> differTypes = new List<ProductType>();
		    foreach (ProductType productType in allType)
		    {
                bool hasFound = false;
		        foreach (ProductType type in productTypeList)
		        {

                    if (productType.TypeId == type.TypeId)
                    {
                        hasFound = true;
                        break;
                    }
                ...
                differTypes.Add(productType);
		    }
		    
			if (differTypes.Count() > 0)
			{
			    foreach (ProductType productType in differTypes)
			    {
                    if (productType.ExFld1 != 1)
                        ProductTypeDao.DeleteById(productType.TypeId);
                    else
                    {
                        // create message which inform that can not delete product type
                    }
			    }
			}
		}
