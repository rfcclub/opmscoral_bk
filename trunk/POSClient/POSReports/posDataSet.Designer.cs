    ...
    public partial class StockStatisticTableAdapter : global::System.ComponentModel.Component {
        ...
        private void InitCommandCollection() {
            ...
            this._commandCollection[0].CommandText = "select type_name, pr.product_id, product_name, color_name, size_name, sum(ifnull(" +
                ...
                "id\r\n    left join (select product_id, sum(quantity) as quantity from department_" +
                "stock_in dstk inner join\r\n      department_stock_in_detail dstkd on dstk.stock_i" +
                "n_id=dstkd.stock_in_id\r\n      where stock_in_date>=@FromDate and stock_in_date<=" +
                "@ToDate and stock_in_type=0\r\n      group by product_id) as instkout on instkout." +
                "product_id=pr.product_id\r\n    left join (SELECT product_id, sum(quantity) as qua" +
                "ntity from stock_out inner join\r\n      stock_out_detail stkd on stock_out.stocko" +
                "ut_id=stkd.stockout_id\r\n      where stkd.defect_status_id=4 and stock_out_date>=" +
                "@FromDate and stock_out_date<=@ToDate\r\n      group by product_id) as tmpout on t" +
                "mpout.product_id=pr.product_id\r\n    left join (SELECT product_id, sum(quantity) " +
                "as quantity from stock_out inner join\r\n      stock_out_detail stkd on stock_out." +
                "stockout_id=stkd.stockout_id\r\n      where stkd.defect_status_id=5 and stock_out_" +
                "date>=@FromDate and stock_out_date<=@ToDate\r\n      group by product_id) as sprtn" +
                " on sprtn.product_id=pr.product_id\r\n    left join (SELECT product_id, sum(quanti" +
                "ty) as quantity from stock_out inner join\r\n      stock_out_detail stkd on stock_" +
                "out.stockout_id=stkd.stockout_id\r\n      where stkd.defect_status_id=8 and stock_" +
                "out_date>=@FromDate and stock_out_date<=@ToDate\r\n      group by product_id) as d" +
                "mg on dmg.product_id=pr.product_id\r\ngroup by type_name, product_name, color_name" +
                ", size_name";
            this._commandCollection[0].CommandType = global::System.Data.CommandType.Text;
            global::MySql.Data.MySqlClient.MySqlParameter param = new global::MySql.Data.MySqlClient.MySqlParameter();
            param.ParameterName = "@FromDate";
