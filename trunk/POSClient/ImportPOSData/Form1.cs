...
namespace ImportPOSData
{
    public partial class Form1 : Form
    {
        ...
        private void btnInput_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = row; i < endRow; i++)
                {
                    ...
                    ImportObject obj = GetImportObject(i, ws, out errorObject);
        ...
        private ImportObject GetImportObject(int row, Worksheet ws, out ErrorObject errorObject)
        {
            ...
            // Type
            Range range = ws.get_Range("B" + row, "B" + row);
            ...
            // Số lượng
            range = ws.get_Range("G" + row, "G" + row);
            name = range.Value2 != null ? range.Value2.ToString() : "";
            value = 0;
        ...
        private void importToDB(ImportObject obj, string stockInId)
        {
            ...
            id = dal.GetSingleValue("Select max(product_id) from product where product_id >= " + dateStr + "00000");
            if (id == null || id.ToString() == string.Empty)
            {
                id = dateStr + "00001";
            }
