...
{
    public class EmployeeWorkingDaysDaoImpl : IEmployeeWorkingDaysDao
    {
        ...
        public IList<EmployeeWorkingDays> FindAll(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ...
                return hibernateCriteria.List<EmployeeWorkingDays>();
            }
            finally 
            {
                if (session != null)
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(EmployeeWorkingDays)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
