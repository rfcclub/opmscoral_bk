    ...
    public class CouponDao : ICouponDao
    {
        ...
        public Coupon Add(Coupon data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Save("CoralPOS.Models.Coupon", data);
                        return data;
                    }
                );
        ...
        public int Update(Coupon data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Update("CoralPOS.Models.Coupon", data);
                        return 0;
                    }
                );
        ...
        public int Delete(Coupon data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Delete("CoralPOS.Models.Coupon", data);
                        return 0;
                    }
                );
		...
		public IList<TClass> FindAllSubProperty<TClass>(LinqCriteria<Coupon> criteria,Func<Coupon,TClass> subProp)
        {
            return (IList<TClass>)HibernateTemplate.Execute(
                                delegate(ISession session)
                                {
                                    ...
                                    QueryHandler<Coupon> handler = new QueryHandler<Coupon>(session);
                                    IList<Coupon> products = handler.GetList(criteria);
