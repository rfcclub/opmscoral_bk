    ...
    public class SimilarProductDao : ISimilarProductDao
    {
        ...
        public SimilarProduct Add(SimilarProduct data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Save("CoralPOS.Models.SimilarProduct", data);
                        return data;
                    }
                );
        ...
        public int Update(SimilarProduct data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Update("CoralPOS.Models.SimilarProduct", data);
                        return 0;
                    }
                );
        ...
        public int Delete(SimilarProduct data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Delete("CoralPOS.Models.SimilarProduct", data);
                        return 0;
                    }
                );
		...
		public IList<TClass> FindAllSubProperty<TClass>(LinqCriteria<SimilarProduct> criteria,Func<SimilarProduct,TClass> subProp)
        {
            return (IList<TClass>)HibernateTemplate.Execute(
                                delegate(ISession session)
                                {
                                    ...
                                    QueryHandler<SimilarProduct> handler = new QueryHandler<SimilarProduct>(session);
                                    IList<SimilarProduct> products = handler.GetList(criteria);
