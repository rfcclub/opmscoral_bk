...
{
    public class MainStockDaoImpl : IMainStockDao
    {
        ...
        public IList<MainStock> FindAll(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ...
                return hibernateCriteria.List<MainStock>();
            }
            finally 
            {
                if (session != null)
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(MainStock)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
