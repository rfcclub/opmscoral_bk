...
{
    public class DepartmentPurchaseOrderDetailLogDaoImpl : IDepartmentPurchaseOrderDetailLogDao
    {
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(DepartmentPurchaseOrderDetailLog)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
