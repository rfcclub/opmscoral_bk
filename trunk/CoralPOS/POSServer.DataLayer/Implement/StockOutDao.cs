    ...
    public class StockOutDao : IStockOutDao
    {
        ...
        public StockOut Add(StockOut data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Save("CoralPOS.Models.StockOut", data);
                        return data;
                    }
                );
        ...
        public int Update(StockOut data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Update("CoralPOS.Models.StockOut", data);
                        return 0;
                    }
                );
        ...
        public int Delete(StockOut data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Delete("CoralPOS.Models.StockOut", data);
                        return 0;
                    }
                );
		...
		public IList<TClass> FindAllSubProperty<TClass>(LinqCriteria<StockOut> criteria,Func<StockOut,TClass> subProp)
        {
            return (IList<TClass>)HibernateTemplate.Execute(
                                delegate(ISession session)
                                {
                                    ...
                                    QueryHandler<StockOut> handler = new QueryHandler<StockOut>(session);
                                    IList<StockOut> products = handler.GetList(criteria);
