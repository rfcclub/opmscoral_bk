    ...
    public class ContractDao : IContractDao
    {
        ...
        public Contract Add(Contract data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Save("CoralPOS.Models.Contract", data);
                        return data;
                    }
                );
        ...
        public int Update(Contract data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Update("CoralPOS.Models.Contract", data);
                        return 0;
                    }
                );
        ...
        public int Delete(Contract data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Delete("CoralPOS.Models.Contract", data);
                        return 0;
                    }
                );
		...
		public IList<TClass> FindAllSubProperty<TClass>(LinqCriteria<Contract> criteria,Func<Contract,TClass> subProp)
        {
            return (IList<TClass>)HibernateTemplate.Execute(
                                delegate(ISession session)
                                {
                                    ...
                                    QueryHandler<Contract> handler = new QueryHandler<Contract>(session);
                                    IList<Contract> products = handler.GetList(criteria);
