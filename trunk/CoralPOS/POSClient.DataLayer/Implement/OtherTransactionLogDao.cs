    ...
    public class OtherTransactionLogDao : IOtherTransactionLogDao
    {
        ...
        public OtherTransactionLog Add(OtherTransactionLog data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Save("CoralPOS.Models.OtherTransactionLog", data);
                        return data;
                    }
                );
        ...
        public int Update(OtherTransactionLog data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Update("CoralPOS.Models.OtherTransactionLog", data);
                        return 0;
                    }
                );
        ...
        public int Delete(OtherTransactionLog data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Delete("CoralPOS.Models.OtherTransactionLog", data);
                        return 0;
                    }
                );
		...
		public IList<TClass> FindAllSubProperty<TClass>(LinqCriteria<OtherTransactionLog> criteria,Func<OtherTransactionLog,TClass> subProp)
        {
            return (IList<TClass>)HibernateTemplate.Execute(
                                delegate(ISession session)
                                {
                                    ...
                                    QueryHandler<OtherTransactionLog> handler = new QueryHandler<OtherTransactionLog>(session);
                                    IList<OtherTransactionLog> products = handler.GetList(criteria);
