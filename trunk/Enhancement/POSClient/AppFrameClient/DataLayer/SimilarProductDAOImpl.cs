...
{
    public class SimilarProductDAOImpl : ISimilarProductDAO
    {
        ...
        {
            return (SimilarProduct) HibernateTemplate.Get(typeof(SimilarProduct), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(SimilarProduct)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
