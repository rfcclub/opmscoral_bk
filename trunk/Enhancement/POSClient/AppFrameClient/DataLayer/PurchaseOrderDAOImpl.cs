...
{
    public class PurchaseOrderDAOImpl : IPurchaseOrderDAO
    {
        ...
        {
            return (PurchaseOrder) HibernateTemplate.Get(typeof(PurchaseOrder), id);
        }
        ...
        {
            PurchaseOrder obj = (PurchaseOrder) HibernateTemplate.Get(typeof (PurchaseOrder), id);
            if (obj != null)
            {
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(PurchaseOrder)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
        ...
        public string SelectMaxPurchaseOrderNumber()
        {
            return (string)
            HibernateTemplate.Execute(
                  delegate(ISession session)
                  {
                      ...
                      IList list = session.CreateCriteria(typeof (PurchaseOrder))
                          .Add(Expression.Like("PurchaseOrderPK.PurchaseOrderId", "%" + shortDateTime + "%"))
                          .AddOrder(Order.Desc("PurchaseOrderPK.PurchaseOrderId"))
                          .SetMaxResults(1)
                          .List();
                      
                      return (list.Count>0 ? ((PurchaseOrder)list[0]).PurchaseOrderPK.PurchaseOrderId : null);
                  }
                );
        }
        ...
        public object SelectSpecificType(ObjectCriteria criteria, IProjection type)
        {
            ...
            try
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(PurchaseOrder)).SetProjection(type); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
