...
{
    public class DepartmentReturnDetailDAOImpl : IDepartmentReturnDetailDAO
    {
        ...
        {
            return (DepartmentReturnDetail) HibernateTemplate.Get(typeof(DepartmentReturnDetail), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(DepartmentReturnDetail)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
