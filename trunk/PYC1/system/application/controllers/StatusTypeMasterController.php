...
class StatusTypeMasterController extends Controller {
	...
	function index() {
	    $this->load->model('StatusTypeMasterModel');
    ...
    function _buildTable($InList, $pageNum, $criteria) {
        ...
        $str .= '<tr class="sort">';
        $str .= '<th> <a class="header_style" href=\'javascript:document.getElementById("action").value ="sort";document.getElementById("hidOrderBy").value ="statusType";document.getElementById("hidPageNum").value ="'.$pageNum.'";document.forms["mainform"].submit();\'>'.$this->lang->line('StatusTypeMaster.statusType').'</a>';
		...
		$str .= '</th>';
	    $str .= '<th> <a class="header_style" href=\'javascript:document.getElementById("action").value ="sort";document.getElementById("hidOrderBy").value ="statusName";document.getElementById("hidPageNum").value ="'.$pageNum.'";document.forms["mainform"].submit();\'>'.$this->lang->line('StatusTypeMaster.statusName').'</a>';
        if ($criteria != null && count($criteria) > 0 && 'statusName' == $criteria['orderField']) {
        	if ($criteria['orderDirection'] == TRUE) {
			 	$str .= '&nbsp;&nbsp;<img src="'.base_url().'/images/arrow_down.png"/>';       		
        	} else {
        		$str .= '&nbsp;&nbsp;<img src="'.base_url().'/images/arrow_up.png"/>';
        ...
        foreach ($InList as $data) {
            ...
            $str .= '<td>'.$data['statusType'].'</td>';
            $str .= '<td>'.$data['statusName'].'</td>';
    ...
    function _doSearch($criteria, $pageSegment = 0) {
		$DataList = $this->StatusTypeMasterModel->findPaging($criteria);
		$countSearch = $this->StatusTypeMasterModel->count($criteria);
        ...
        if (count($DataList) == 0) {
            ...
            $config['base_url'] = base_url().'index.php?c=statustypemastercontroller&action=next';
    ...
    function _search() {
		...
		ObjectCriteria::addEqCriteria($criteria, ColumnHelper::mapContractColumn('statusType'), $this->input->post('txtStatusType'));
		ObjectCriteria::addEqCriteria($criteria, ColumnHelper::mapContractColumn('statusName'), $this->input->post('txtStatusName'));
		ObjectCriteria::addOrder($criteria, ColumnHelper::mapContractColumn('statusType'), FALSE);
