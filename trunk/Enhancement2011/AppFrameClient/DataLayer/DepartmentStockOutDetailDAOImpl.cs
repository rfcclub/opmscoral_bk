...
{
    public class DepartmentStockOutDetailDAOImpl : IDepartmentStockOutDetailDAO
    {
        ...
        {
            return (DepartmentStockOutDetail) HibernateTemplate.Get(typeof(DepartmentStockOutDetail), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(DepartmentStockOutDetail)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
