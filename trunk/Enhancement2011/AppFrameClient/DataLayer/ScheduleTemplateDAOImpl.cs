...
namespace AppFrame.DataLayer
{
    public class ScheduleTemplateDAOImpl : IScheduleTemplateDAO
    {
        ...
        {
            return (ScheduleTemplate) HibernateTemplate.Get(typeof(ScheduleTemplate), id);
        }
        ...
        private int Count(ObjectCriteria criteria)
        {
            ...
            try 
            {
                ICriteria hibernateCriteria = session.CreateCriteria(typeof(ScheduleTemplate)).SetProjection(Projections.RowCount()); ;
                if (criteria != null)
                {
                    IDictionary<string, SubObjectCriteria> map = criteria.GetSubCriteria();
                    if (map.Count > 0)
