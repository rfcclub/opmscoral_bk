    ...
    public class EmployeeWorkingDaysDao : IEmployeeWorkingDaysDao
    {
        ...
        public EmployeeWorkingDays Add(EmployeeWorkingDays data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Save("CoralPOS.Models.EmployeeWorkingDays", data);
                        return data;
                    }
                );
        ...
        public int Update(EmployeeWorkingDays data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Update("CoralPOS.Models.EmployeeWorkingDays", data);
                        return 0;
                    }
                );
        ...
        public int Delete(EmployeeWorkingDays data)
        {
			_hibernateTemplate.Execute(delegate(ISession session) 
                    {
                        session.Delete("CoralPOS.Models.EmployeeWorkingDays", data);
                        return 0;
                    }
                );
		...
		public IList<TClass> FindAllSubProperty<TClass>(LinqCriteria<EmployeeWorkingDays> criteria,Func<EmployeeWorkingDays,TClass> subProp)
        {
            return (IList<TClass>)HibernateTemplate.Execute(
                                delegate(ISession session)
                                {
                                    ...
                                    QueryHandler<EmployeeWorkingDays> handler = new QueryHandler<EmployeeWorkingDays>(session);
                                    IList<EmployeeWorkingDays> products = handler.GetList(criteria);
